...
Every pull request automatically gets a preview environment with full passkey support:

- **Automatic deployment** on PR creation/updates
- **Unique URLs** for testing: `villa-shard-{pr}-{date}-{service}.up.railway.app`
- **Full passkey authentication** (HTTPS enabled)
- **Auto-cleanup** when PR is closed

See [PR_PREVIEW_ENVIRONMENTS.md](PR_PREVIEW_ENVIRONMENTS.md) for complete documentation.
...
| "ERR*NGROK*\*"     | Auth token issue      | `ngrok config add-authtoken YOUR_TOKEN`  |
