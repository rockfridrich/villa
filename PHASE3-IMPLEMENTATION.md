...
- name: Select tests based on changes
  id: select
  run: |
    TESTS=$(pnpm --filter @villa/hub exec tsx ../../scripts/select-e2e-tests.ts)
    echo "tests=$TESTS" >> $GITHUB_OUTPUT
...
- name: Run E2E tests
  run: |
    if [ "${{ steps.select.outputs.tests }}" = "all" ]; then
      ...
      pnpm --filter @villa/hub test:e2e:chromium ${{ steps.select.outputs.tests }}
    fi
```
...
pnpm --filter @villa/hub exec tsx ../../scripts/select-e2e-tests.ts
