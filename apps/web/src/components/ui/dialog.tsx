...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ...
      className={clsx(
        // Base styles
        'fixed z-50 grid w-full gap-4 border border-neutral-200 bg-cream p-6 shadow-lg duration-200',
        // Mobile: Bottom sheet (slide up from bottom)
        'bottom-0 left-0 right-0 rounded-t-lg',
        'data-[state=open]:animate-in data-[state=closed]:animate-out',
        'data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0',
        'data-[state=closed]:slide-out-to-bottom data-[state=open]:slide-in-from-bottom',
        // Desktop: Centered modal (above sm breakpoint)
    ...
    >
      {children}
      <DialogPrimitive.Close className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-cream transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-accent-yellow focus:ring-offset-2 disabled:pointer-events-none">
        <X className="h-4 w-4" />
