...
export function normalizeAvatar(
  avatar: Avatar | AvatarConfig | CustomAvatar,
): Avatar {
...
export async function getAvatarUrl(
  ...
  // Generated avatar
  const config = isGeneratedAvatar(avatar)
    ? avatar.config
    : (avatar as AvatarConfig);

  if (format === "png") {
    return generateAvatarPng(
      walletAddress,
      "selection" in config ? config.selection : "other",
      "variant" in config ? config.variant : 0,
      size,
    );
  }

  // SVG format
  const svg = generateAvatarFromSelection(
    walletAddress,
    "selection" in config ? config.selection : "other",
    "variant" in config ? config.variant : 0,
  );
